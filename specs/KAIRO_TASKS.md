...
**Done so far:** Baseline `kairo-project.json` fixture generated from the full Scott scene pipeline (116 records: 28 auto-accepted, 88 uncertain). Checked in at `docs/examples/scott-p736-first-review.kairo-project.json`. Added reviewer queue artifacts at `docs/examples/scott-p736-first-review-queue.md` and `.csv`, generated by `pnpm review:first-queue`, to make the 88-record human visual pass auditable. Regression coverage in `scottP736FirstReview.test.ts` now checks fixture parse/library summary, project round-trip, and queue alignment. Remaining: human visual QA pass on the ambiguous geometry associations (especially the 4 protected P736 labels) and upgrade to a human-reviewed version of the fixture.
