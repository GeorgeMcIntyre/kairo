...
**Done so far:** Added a mesh-only named colored part probe at `tools/glb-probes/jt-bridge-part-output.glb`, generated by `build-jt-bridge-part-probe.mjs` and inspected by `inspect-jt-bridge-part-probe.mjs`. The probe is one red 100 x 50 x 25 mm block named `jt_bridge_probe_named_red_block_100x50x25mm`, with one mesh, one material, 12 triangles, normals, part-number metadata, and `extras` on scene/node/mesh/primitive/material. Local inspection passes all structural/name/color/dimension checks. Added `pnpm smoke:cad-glb-probes`, which rebuilds and inspects both the GLB primitive compatibility probe and the JT bridge named-part probe, producing `probe-output.glb` (18,336 bytes) and `jt-bridge-part-output.glb` (3,952 bytes). Manual CAD Exchanger, reopened-JT, JT2Go, and Process Simulate result matrix lives in `docs/CAD_EXCHANGER_JT_BRIDGE_PROBE.md`. Remaining: George runs the desktop bridge test and fills in the matrix before any exporter/bridge implementation.
...
**Current automated preflight:** `pnpm smoke:current-gates` runs the non-manual gate set and reports the manual gates that remain outside automation.
...
**Done so far:** Baseline `kairo-project.json` fixture generated from the full Scott scene pipeline (116 records: 28 auto-accepted, 88 uncertain). Checked in at `docs/examples/scott-p736-first-review.kairo-project.json`. Added reviewer queue artifacts at `docs/examples/scott-p736-first-review-queue.md` and `.csv`, generated by `pnpm review:first-queue`, to make the 88-record human visual pass auditable. Added `pnpm review:first-check` and strict gate `pnpm review:first-require-complete`; the strict gate currently fails as expected because the 88 generated-uncertain rows have no reviewer evidence yet. Regression coverage in `scottP736FirstReview.test.ts` now checks fixture parse/library summary, project round-trip, and queue alignment. Remaining: human visual QA pass on the ambiguous geometry associations (especially the 4 protected P736 labels) and upgrade to a human-reviewed version of the fixture.
...
**Done so far:** Added `pnpm run build:cloudflare`, backed by `scripts/prepare-cloudflare-pages-dist.mjs`, to build the viewer and prune local staged-scene assets from `apps/viewer/dist/scenes`. The script verifies `_redirects` and fails oversized remaining static assets. `pnpm run build:cloudflare` passed and left only `index.html`, `assets/`, and `_redirects` in `apps/viewer/dist`. Added short checklist `specs/DEPLOY_CLOUDFLARE.md`; updated canonical deployment docs. Cleared stale local Wrangler cache files that pointed Pages commands at the wrong account; `npx wrangler pages project list` now succeeds. Direct upload deployed `apps/viewer/dist` to `kairo-viewer`: immutable URL `https://b5859425.kairo-viewer-6lh.pages.dev`, branch alias `https://codex-kairo-dxf-block-instan.kairo-viewer-6lh.pages.dev`. HTTP smoke passed for the URL, SPA fallback, JS bundle, and CSS bundle. Added `pnpm smoke:pages` (`scripts/smoke-pages-demo.mjs`) and verified the branch alias app shell, JS/CSS assets, current viewer control labels in the deployed JS bundle, SPA fallback, and Scott staged-manifest pruning. The viewer demo action now loads the bundled sample scene instead of a pruned staged Scott asset. `specs/KAIRO_PUBLIC_DEMO_CHECKLIST.md` lists the remaining public-demo and `.kairo` upload browser gates. Remaining: browser/canvas smoke test against the Pages URL.
